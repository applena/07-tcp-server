...
let directMessage = (data, userId, socket) => {
  if(socketPool[data.target]){
    let target = socketPool[data.target];
    target.socket.write(`<${socketPool[userId].nickname}>: ${data.payload}\n`);
  } else { socket.write('user does not exist');}
};
